...
public class DataWarehouseRepository {
    ...
    public CampusReport getCampusReport(String code, LocalDate from, LocalDate to) {
        ...
        Map<String, List<VdxBorrowingSummary>> borrowingSummaries = vdxRepo.getBorrowingSummary(code, from, to)
                .collect(Collectors.groupingBy(VdxBorrowingSummary::getReqName));
        Map<String, List<VdxLendingSummary>> lendingSummaries = vdxRepo.getLendingSummary(code, from, to)
        ...
        libraries.forEach((String name) -> {
            addInstitution(campus, code, name, borrowingSummaries.getOrDefault(name, Collections.emptyList()), lendingSummaries.getOrDefault(name, Collections.emptyList()));
        });
